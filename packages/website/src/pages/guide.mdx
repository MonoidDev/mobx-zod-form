...
Use the `partial` token to partially determine the initial values of an object, leaving other fields using `empty`. `partial` does not work for types other than objects.

It is useful when you have a complicated object but just a couple of fields have their initial output.
